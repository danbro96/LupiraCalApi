...
public sealed class AvailabilityTests(CalApiTestFactory factory) : IntegrationTest(factory)
{
    ...
    public async Task A_day_can_hold_a_whole_day_and_a_timed_segment()
    {
        ...
        {
            CalendarId = calId, Title = "Office", Kind = "Availability", Availability = AvailabilityStatus.Office,
            IsAllDay = true, StartDate = day, EndDate = day.AddDays(1),
        });
        var home = await PostAsync(api, new CreateCalendarItemRequest
        {
            CalendarId = calId, Title = "Home", Kind = "Availability", Availability = AvailabilityStatus.Home,
        ...
        Assert.Equal(AvailabilityStatus.Office, office.KindDetails?.Availability?.Status);
        Assert.Equal(AvailabilityStatus.Home, home.KindDetails?.Availability?.Status);
    ...
    public async Task Availability_status_can_be_updated()
    {
        ...
        {
            CalendarId = calId, Title = "Status", Kind = "Availability", Availability = AvailabilityStatus.Office,
        ...
        Assert.Equal(AvailabilityStatus.Sick, got!.KindDetails?.Availability?.Status);
    }
    ...
    public async Task A_recurring_default_week_expands_to_each_occurrence()
    {
        ...
        {
            CalendarId = calId, Title = "Office hours", Kind = "Availability", Availability = AvailabilityStatus.Office,
