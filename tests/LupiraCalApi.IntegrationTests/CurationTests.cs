...
public sealed class CurationTests(CalApiTestFactory factory) : IntegrationTest(factory)
{
    ...
    public async Task Proposed_item_is_invisible_over_dav_until_accepted()
    {
        ...
        var itemUrl = $"/dav/u/{uid}/cal/{calId}/{item.ExternalId}.ics";
        ...
        Assert.DoesNotContain(listed.Descendants(D + "href"), h => h.Value.Contains(item.ExternalId));
        ...
        Assert.Contains(after.Descendants(D + "href"), h => h.Value.Contains(item.ExternalId));
    }
    ...
    public async Task Item_accepted_into_two_calendars_is_visible_in_both()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, (await dav.GetAsync($"/dav/u/{uid}/cal/{cal1}/{item.ExternalId}.ics")).StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await dav.GetAsync($"/dav/u/{uid}/cal/{cal2}/{item.ExternalId}.ics")).StatusCode);
    }
    ...
    public async Task Add_with_accepted_status_is_immediately_visible()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, (await dav.GetAsync($"/dav/u/{uid}/cal/{calId}/{item.ExternalId}.ics")).StatusCode);
    }
